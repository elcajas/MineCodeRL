...
class PPOagent:
    ...
    def get_features(self, images: torch.Tensor):
        ...
        if isinstance(self.policy_model.module.image_model, MineCLIP):
            return self.policy_model.module.image_model.forward_image_features(images.to(self.device))
        
        if isinstance(self.policy_model.module.image_model, ImageEncoder):
    ...
    def get_action_and_value(self, batch, action=None):
        ...
        dist = self.policy_model.module.actor.dist_fn(*logits) if isinstance(logits, tuple) else self.policy_model.module.actor.dist_fn(logits)
        if action is None:
            action = dist.mode() if self.policy_model.module.actor._deterministic_eval and not self.policy_model.training else dist.sample()
