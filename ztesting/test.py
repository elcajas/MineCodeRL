from agents.inference import load_model, load_image, process, annotate, load_model_with_lora
import cv2
import imageio
import os
import logging
...
def annotation(args):
    ...
    for ind, image_file in enumerate(sorted(image_files, key=numerical_sort)):
        
        IMAGE_PATH = os.path.join(FOLDER_PATH, image_file)
        image_source, image = load_image(IMAGE_PATH)

        boxes, logits, phrases = process(
            model=model,
            image=image,
            caption=TEXT_PROMPT,
        ...
        # Filter logits and phrases with logits greater than 0.5
        ...
        # Update total detections and logit sum
        ...
        output_path = os.path.join(IMAGES_OUTPUT_PATH, f"{laterpath}_{ind}.png")
        cv2.imwrite(output_path, annotated_frame)

        logging.info(f"Image{ ind}: {', '.join(f'{phrase} {logit:.2f}' for phrase, logit in zip(phrases, logits))}")

        print(f"Image{ ind}:", ", ".join(f"{phrase} {logit:.2f}" for phrase, logit in zip(phrases, logits)))
